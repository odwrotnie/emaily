...
/**
  * This is an event sourced entity. It has a state, [[EmailyState]], which
  ...
  */
class EmailyEntity extends PersistentEntity {

  override type Command = EmailyCommand[_]
  override type Event = EmailyEvent
  override type State = EmailyState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: EmailyState = EmailyState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case EmailyState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        EmailyState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class EmailyState(message: String, timestamp: String)
...
object EmailyState {
  /**
    ...
    */
  implicit val format: Format[EmailyState] = Json.format
}

/**
  * This interface defines all the events that the EmailyEntity supports.
  */
sealed trait EmailyEvent extends AggregateEvent[EmailyEvent] {
  def aggregateTag = EmailyEvent.Tag
}

object EmailyEvent {
  val Tag = AggregateEventTag[EmailyEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends EmailyEvent
...
/**
  ...
  */
sealed trait EmailyCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends EmailyCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends EmailyCommand[String]
...
/**
  ...
  * application loader.
  */
object EmailySerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[EmailyState]
  )
}
